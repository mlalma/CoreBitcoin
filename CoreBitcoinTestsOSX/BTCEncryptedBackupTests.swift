...
class BTCEncryptedBackupTests: XCTestCase {
    ...
    func testShortBackup() {
        let timestamp: TimeInterval = 1427720967
        
        let plaintext = "The Times 03/Jan/2009 Chancellor on brink of second bailout for banks".data(using: String.Encoding.utf8, allowLossyConversion: false)
        
        let masterKey: Data = BTCSHA256("Master Key".data(using: String.Encoding.utf8, allowLossyConversion: false))! as Data
        XCTAssertEqual(masterKey,
                       BTCDataFromHex("08c17482950a872178b8030c8f8a63bc6e5f9f680dd25739e1ec7e0b544f40f9"), "master key")
        let backupKey = BTCEncryptedBackup.backupKey(for: BTCNetwork.mainnet(), masterKey: masterKey)
        let backupKeyTestnet = BTCEncryptedBackup.backupKey(for: BTCNetwork.testnet(), masterKey: masterKey)
        
        XCTAssertEqual(backupKey, BTCDataFromHex("7618f25cd5faadd52d0ea3b608b0c076664f5816b81311017985ae229157057a"),
                       "must produce a determinstic backup key")
        XCTAssertEqual(backupKeyTestnet, BTCDataFromHex("caa57de4c3d9c77186175fbfdc326997162da0ce1b74022a51c600838449b2c3"),
                       "must produce a determinstic backup key")
        
        let backup: BTCEncryptedBackup = BTCEncryptedBackup.encrypt(plaintext, backupKey: backupKey, timestamp: timestamp)
        
        XCTAssertNotNil(backup, "Should encrypt alright")
        
        XCTAssertGreaterThanOrEqual(backup.encryptedData.count, 1 + 4 + 16 + 1 + plaintext!.count + 1 + 68, "Should be of realistic length")
        ...
        XCTAssertEqual(backup.authenticationKey.privateKey! as Data,
                       BTCDataFromHex("44b45878c33c974179f5363fee95f9e9d4a60c97e9c865e58b57bef3558034f4"),
                       "Should use deterministic authentication private key")
        XCTAssertEqual(backup.authenticationKey.publicKey! as Data,
                       BTCDataFromHex("028747be6de07552c48f9db23617792d47df1accd611175f6dfe636f4098984a09"),
                       "Should use deterministic compressed authentication public key")
        ...
        let backup2: BTCEncryptedBackup = BTCEncryptedBackup.decrypt(backup.encryptedData, backupKey: backupKey)
    ...
    func testLongBackup() {
        ...
        let plaintext: Data?
        do {
            plaintext = try JSONSerialization.data(withJSONObject: backupJSON, options: JSONSerialization.WritingOptions(rawValue: 0))
        } catch {
            XCTFail("Error: \(error)")
            plaintext = nil
        }
        
        let toPrint = NSString.init(data: plaintext!, encoding: String.Encoding.utf8.rawValue)
        print("plaintext = \(String(describing: toPrint))")
        ...
        let backup: BTCEncryptedBackup = BTCEncryptedBackup.encrypt(plaintext, backupKey: backupKey, timestamp: 1427720967.0)
