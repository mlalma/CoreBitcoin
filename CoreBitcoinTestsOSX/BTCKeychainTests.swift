...
class BTCKeychainTests: XCTestCase {
    ...
    func testPaths() {
        ...
        let keychain: BTCKeychain = BTCKeychain(extendedKey:
            "xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi")
        
        XCTAssertEqual(keychain.derivedKeychain(withPath: "").extendedPublicKey, keychain.extendedPublicKey, "must return root key")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "m").extendedPublicKey, keychain.extendedPublicKey, "must return root key")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "/").extendedPublicKey, keychain.extendedPublicKey, "must return root key")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "m//").extendedPublicKey, keychain.extendedPublicKey, "must return root key")
        
        XCTAssertEqual(keychain.derivedKeychain(withPath: "m/0'").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: true).extendedPublicKey, "must return hardened child at index 0")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "/0'").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: true).extendedPublicKey, "must return hardened child at index 0")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "0'").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: true).extendedPublicKey, "must return hardened child at index 0")
        
        XCTAssertEqual(keychain.derivedKeychain(withPath: "m/0").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: false).extendedPublicKey, "must return non-hardened child at index 0")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "/0").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: false).extendedPublicKey, "must return non-hardened child at index 0")
        XCTAssertEqual(keychain.derivedKeychain(withPath: "0").extendedPublicKey,
                       keychain.derivedKeychain(at: 0, hardened: false).extendedPublicKey, "must return non-hardened child at index 0")
        
        XCTAssertNil(keychain.derivedKeychain(withPath: "m / 0 / 1"), "must return nil if path contains spaces")
        XCTAssertNil(keychain.derivedKeychain(withPath: "m/b/c"), "must return nil if path contains irrelevant characters")
        XCTAssertNil(keychain.derivedKeychain(withPath: "1/m/2"), "must return nil if path contains irrelevant characters")
        XCTAssertNil(keychain.derivedKeychain(withPath: "m/1.2^3"), "must return nil if path contains irrelevant characters")
    ...
    func testVector1() {
        ...
        */
        ...
        let masterChain: BTCKeychain = BTCKeychain(seed: seed)
...
//        println("identifier: \(masterChain.identifier) fingerprint: \(masterChain.fingerprint)")
        ...
        let m0prv: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0'")
        ...
        let m0prv1pub: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0'/1")
        ...
        let m0prv1pub2prv: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0'/1/2'")
        ...
        let m0prv1pub2prv2pub: BTCKeychain = m0prv1pub2prv.derivedKeychain(at: 2)
        ...
        let m0prv1pub2prv2pub1Gpub: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0'/1/2'/2/1000000000")
...
//        println("identifier: \(masterChain.identifier) fingerprint: \(masterChain.fingerprint)")
        ...
        let m0pub: BTCKeychain = masterChain.derivedKeychain(at: 0)
        ...
        let m0pubFFprv: BTCKeychain = m0pub.derivedKeychain(at: 2147483647, hardened: true)
        ...
        let m0pubFFprv1: BTCKeychain = m0pubFFprv.derivedKeychain(at: 1)
        ...
        let m0pubFFprv1pubFEprv: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0/2147483647'/1/2147483646'")
        ...
        let m0pubFFprv1pubFEprv2: BTCKeychain = masterChain.derivedKeychain(withPath: "m/0/2147483647'/1/2147483646'/2")
    ...
    func testZeroPaddedPrivateKeys() {
        let keychain: BTCKeychain = BTCKeychain(seed: "stress test".data(using: String.Encoding.utf8, allowLossyConversion: false))
        
        for i in 0 ..< 2*256 {
            //#puts i # i=70 yields a zero-prefixed private key.
            let key: BTCKey = keychain.key(at: UInt32(i), hardened: true)
...
//            }
        ...
        XCTAssertEqual(keychain.key(at: 70, hardened: true).address.string, "1FZQfsXwAoUcn9WVwbfRb4jMMkPJEozLWH", "")
        XCTAssertEqual(keychain.key(at: 70, hardened: true).privateKey.bytes.assumingMemoryBound(to: uint8.self)[0], 0, "must be zero-prefixed")
        XCTAssertEqual(keychain.key(at: 227, hardened: true).address.string, "1LRbeWJC3sLGRk7ob82djVYTNhsH2UdR4f", "")
        XCTAssertEqual(keychain.key(at: 227, hardened: true).privateKey.bytes.assumingMemoryBound(to: uint8.self)[0], 0, "must be zero-prefixed")
        XCTAssertEqual(keychain.key(at: 455, hardened: true).address.string, "1HSr4B5Hr3hc7vAzNHbp7SV7rsFzUhQSeF", "")
        XCTAssertEqual(keychain.key(at: 455, hardened: true).privateKey.bytes.assumingMemoryBound(to: uint8.self)[0], 0, "must be zero-prefixed")
    ...
    func verifyDeserialization(keychain: BTCKeychain) { //Not prefixed with `test` because we don't want Xcode to automatically run this
        ...
        let pubchain: BTCKeychain = BTCKeychain(extendedKey: keychain.extendedPublicKey)
        let prvchain: BTCKeychain = BTCKeychain(extendedKey: keychain.extendedPrivateKey)
        ...
        XCTAssertEqual(prvchain.key(at: 123), pubchain.key(at: 123), "both chains should be able to derive the same key")
    }
